...
## 🆕 v1.1 更新（2026-08-24）
- **新增 `gen_anki_enhanced.py`**：易错强化 v4 最终版 — 强关联引擎 v2（三信号全库检测 → 48 概念家族）、🔴红盒/🟣紫盒/🔵蓝盒三层错题标注、⚡跨题型关联/正误反转、⚠️非全选多选提示、🌐联网权威助记（事故分级/急救数字/两票三制）、本机答错计数器（localStorage）
- **新增 `gen_anki_from_print_sheet.py`**：支持「打印」sheet → 单模板 + 题型角标 + 主题区块
- **升级模板 V10.1**：注释盒全部用选项原文引用（乱序下永远指向正确项）、判断题固定 A正确/B错误不洗牌
- **进度保留保证**：笔记 GUID 与用户集合一致 → 导入零新增卡；模型 ID 对齐（1781190129778）→ 模板不分裂；卡组名动态读取（\x1f 归一化 ::）
